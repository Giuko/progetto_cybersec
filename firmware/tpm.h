...
}  __attribute__((packed)) TPMS_RSA_PARMS;
...
}  __attribute__((packed)) TPMT_PUBLIC;

typedef struct {
    uint16_t size;                    // Total size of publicArea
    TPMT_PUBLIC publicArea;
}  __attribute__((packed)) TPM2B_PUBLIC;
